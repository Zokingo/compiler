...
//_
	...
	//终极符时的处理，term是分析栈，栈顶(分析节点)
	//终结符包括关键字、运算符、界符、id和四种常量
	private void termOP(String term){
		if(firstWord.type.equals(Word.INT_CONST)||firstWord.type.equals(Word.CHAR_CONST)||
			...
			analyseStack.remove(0);//将分析栈中当前符移出
			wordList.remove(0);//将单词流中对应内容移出
		}
		else{
			errorCount++;
			analyseStack.remove(0);
	...
	private void nonTermOP(String nonTerm){
		...
		case 'S':
			if(firstWord.value.equals("void")){
				...
				analyseStack.remove(0);
				wordList.remove(0);
				error=new Error(errorCount,"case S:主函数没有返回值",firstWord.line,firstWord);
				errorList.add(error);	
				graErrorFlag=true;
			}
			break;
		...
		case 'Y':
			if(firstWord.value.equals("int")){
				...
				analyseStack.remove(0);
				analyseStack.add(0,new AnalyseNode(AnalyseNode.TERMINAL, "float", null));
			}
			else{
				errorCount++;
				analyseStack.remove(0);
				wordList.remove(0);
				error=new Error(errorCount,"case Y:非法数据类型",firstWord.line,firstWord);
				errorList.add(error);	
				graErrorFlag=true;
			}
			break;
		case 'Z'://添加P支持赋值有错
			if(firstWord.type.equals(Word.IDENTIFIER)||firstWord.type.equals(Word.INT_CONST)||firstWord.type.equals(Word.CHAR_CONST)||
				...
				analyseStack.remove(0);
				wordList.remove(0);
				error=new Error(errorCount,"case Z:非法标识符",firstWord.line,firstWord);
				errorList.add(error);
				graErrorFlag=true;
			}
			break;
		...
		case 'U':
			if(firstWord.type.equals(Word.IDENTIFIER)){
				...
				analyseStack.remove(0);
				wordList.remove(0);
				error=new Error(errorCount,"case U:非法标识符",firstWord.line,firstWord);
				errorList.add(error);
				graErrorFlag=true;
			}
			break;
		...
		case 'E':
			if(firstWord.type.equals(Word.IDENTIFIER)){
				...
				analyseStack.remove(0);
				wordList.remove(0);
				error=new Error(errorCount,"case E:不能进行算术运算的数据类型",firstWord.line,firstWord);
				errorList.add(error);	
				graErrorFlag=true;
			}
			break;
		...
		case 'H':
			if(firstWord.type.equals(Word.IDENTIFIER)){
				...
				analyseStack.remove(0);
				wordList.remove(0);
				error=new Error(errorCount,"case H:不能进行算术运算的数据类型",firstWord.line,firstWord);
				errorList.add(error);	
				graErrorFlag=true;
			}
			break;
		...
		case 'D':
			if(firstWord.value.equals("==")){
				...
				analyseStack.remove(0);
				wordList.remove(0);
				error=new Error(errorCount,"case D:非法运算符",firstWord.line,firstWord);
				errorList.add(error);	
				graErrorFlag=true;
			}
			break;
		...
		case 'L':
			if(firstWord.type.equals(Word.IDENTIFIER)){
			...
			else if(firstWord.type.equals(Word.CHAR_CONST)){//char类型常量赋值
				analyseStack.remove(0);
				analyseStack.add(0,new AnalyseNode(AnalyseNode.TERMINAL, "ch", null));
			}
			else if(firstWord.type.equals(Word.FLOAT_CONST)){//char类型常量赋值
				analyseStack.remove(0);
				analyseStack.add(0,new AnalyseNode(AnalyseNode.TERMINAL, "floatnum", null));
			}
			else if(firstWord.value.equals("(")){
				...
				analyseStack.remove(0);
				wordList.remove(0);
				error=new Error(errorCount,"case L:算术运算不能进行算术运算的数据类型或括号不匹配",firstWord.line,firstWord);
				errorList.add(error);
				graErrorFlag=true;
			}
			break;
		...
		case 'T':
			if(firstWord.type.equals(Word.IDENTIFIER)){
				...
				analyseStack.remove(0);
				wordList.remove(0);
				error=new Error(errorCount,"case T:不能进行算术运算的数据类型",firstWord.line,firstWord);
				errorList.add(error);	
				graErrorFlag=true;
			}
			break;
	...
	private void actionSignOP(){
		if(top.name.equals("@ADD_SUB")){
			...
			semanticStack.push(U.value);//为什么
			analyseStack.remove(0);
